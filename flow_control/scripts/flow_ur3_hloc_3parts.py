...
def main(cfg):
    ...
    part_info_file = root_dir / 'parts.json'
    with open(part_info_file, 'r') as f_obj:
        parts_info = json.load(f_obj)
    ...
    for idx in range(3):
        ...
        episode_name, _, res_best = selection_hloc.get_best_demo(query_cam, part_name=part_map[idx])
        rec_path = root_dir / episode_name

        live_state, _, _, live_info = env.step(None)

        control_config = dict(mode="pointcloud-abs", threshold=0.25, threshold_far=1.5)
        servo_module = ServoingModule(rec_path, control_config=control_config, start_paused=True,
                                      ...
                                      load=parts_info[episode_name][part_map[idx]])
