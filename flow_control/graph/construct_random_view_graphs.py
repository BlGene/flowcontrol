...
def create_single_demo_graph(recordings: list, demo_idx: int):
    ...
    dist_cmb = get_cmb_trafo(recordings[demo_idx])
    ...
    for frame_idx in range(get_len(recordings[demo_idx])):
        ...
        demoframeidx2node[str((demo_idx, frame_idx))] = node_idx
        ...
        node_times.append(node_idx)
        node_idx += 1

    # Construct edges
    for i in tqdm(curr_demo_node_idcs):
        ...
        demo_idx, frame_idx, = node_idx2frame[i]
        closest_frame_idcs = np.argpartition(dist_cmb[frame_idx], 2)

        closest_node_idx = demoframeidx2node[str((demo_idx, closest_frame_idcs[1]))]

        second_closest_node_idx = demoframeidx2node[str((demo_idx, closest_frame_idcs[random.randint(2,7)]))]

        triplet_edges.append((i, closest_node_idx, second_closest_node_idx))
...
if __name__ == "__main__":
    ...
    recordings = get_demonstrations(data_dir=os.path.join(params.paths.dataroot, params.preprocessing.raw_data),
                                    ...
                                    file_prefix=params.preprocessing.file_prefix,)
